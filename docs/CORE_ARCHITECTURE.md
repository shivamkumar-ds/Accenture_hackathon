...
9. **Architecture evolves from evidence, not speculation.** New architectural complexity must be justified by production telemetry, customer feedback, measured performance bottlenecks, or repeated operational pain — not by "might be useful someday" or "another product does this." Deferred ideas (§10) stay documented and get revisited only when evidence demands it. Added after this document's first real implementation cycle (Phase A instrumentation, the Bid Decision feature, and the Compliance Verification UI) surfaced the gap: the first eight principles govern what the AI does, but none of them governed when the architecture itself is allowed to change. This one does.
...
Cache at the level of the atomic object, not the document. A Verdict is cached keyed on **(Requirement Pattern, Evidence Version, Evaluation Logic Version)** — not on the tender document as a whole. In the current implementation, Evidence Version is represented by a content hash of the underlying evidence; the architecture only requires that some version identifier exists, not that it be a hash specifically — a document revision number, a Capability Graph version ID, or a database row version would satisfy the same concept if the implementation ever changes. Government tenders from the same department frequently reuse near-identical requirement templates; once a Verdict has been computed for "does this company hold ISO 9001" against the company's current evidence, a structurally similar requirement appearing in a different tender should reuse that Verdict rather than trigger fresh reasoning.
