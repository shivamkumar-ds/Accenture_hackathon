...
const REMEDIATION_SECTIONS: {
  ...
  },
  {
    ...
    emptyCopy: "System coverage is complete -- BidOps could evaluate every requirement.",
    note: () => "BidOps could not fully evaluate these requirements yet. This reflects a system limitation, not a confirmed company deficiency.",
  },
  {
...
export default function Evaluation() {
  ...
  // Positive Factors (Analysis tab) -- collapsed by default regardless
  // of item count, unlike the REMEDIATION_SECTIONS cards above (which
  // default open when they have items). This section is supporting
  // evidence, not the thing the user needs to investigate -- see the
  // placement rationale in the Positive Factors card below.
  const [positiveFactorsOpen, setPositiveFactorsOpen] = useState(false);
  ...
  // Positive Factors (Analysis tab) -- MET compliance-matrix rows,
  // joined against the already-loaded tender requirements by
  // requirement_id. Read-only presentation over existing data: no new
  // API call, no new backend field, no schema change. A row without a
  // matching requirement or without a description is skipped rather
  // than given an invented label -- the same "never fabricate" rule
  // the rest of this page already follows for gap items (see e.g.
  // build_unsupported_coverage_result()'s docstring on the backend).
  // Deduplicated defensively by requirement_id even though
  // decision_service.py writes exactly one ComplianceMatrix row per
  // requirement per recommendation today (verified before writing
  // this) -- cheap to guard, and it costs nothing if it never fires.
  const metPositiveFactors = useMemo(() => {
    const requirementById = new Map(requirements.map((r) => [r.id, r]));
    const seen = new Set<string>();
    const out: { requirement_id: string; requirement_type: RequirementType; description: string }[] = [];
    for (const row of data?.compliance_matrix ?? []) {
      if (row.status !== "met" || seen.has(row.requirement_id)) continue;
      const req = requirementById.get(row.requirement_id);
      if (!req || !req.description) continue;
      seen.add(row.requirement_id);
      out.push({ requirement_id: row.requirement_id, requirement_type: req.requirement_type, description: req.description });
    }
    return out;
  }, [data, requirements]);
            ...
            return (
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="relative rounded-xl border bg-surface p-6 sm:p-8 shadow-hero overflow-hidden">
                      ...
                      {/* The four-question ladder (architecture debate
                          ...
                          -> Did BidOps fully evaluate everything? -> Does
                ...
                {assessmentTab === "analysis" && (
                  <div className="space-y-6">
                    {/* Positive Factors -- placed first, and visually
                        distinct (success tone, collapsed by default,
                        never mixed into the REMEDIATION_SECTIONS list
                        below) so the tab reads as two things: what
                        supports the decision, then what needs
                        attention. Reuses the exact Card/CardHeader/
                        CardBody + expand-collapse pattern the gap
                        sections already use -- no new visual
                        component. Omitted entirely when there are no
                        MET requirements, rather than rendering an
                        empty card. */}
                    {metPositiveFactors.length > 0 && (
                      <Card className="border-success/30">
                        <CardHeader
                          ...
                          description={`${metPositiveFactors.length} item${metPositiveFactors.length === 1 ? "" : "s"} -- capability requirements the company currently satisfies.`}
                        />
                        <CardBody className="!py-2 divide-y divide-border -mx-6">
                          <button
                            type="button"
                            onClick={() => setPositiveFactorsOpen((prev) => !prev)}
                            className="w-full flex items-center justify-between gap-3 px-6 py-2 text-left text-xs font-medium text-muted-foreground"
                          >
                            {positiveFactorsOpen ? "Hide items" : "Show items"}
                            <ChevronDown size={14} className={cn("transition-transform", positiveFactorsOpen && "rotate-180")} />
                          </button>
                          {positiveFactorsOpen && (
                            <ul className="divide-y divide-border">
                              {metPositiveFactors.map((f) => (
                                <li key={f.requirement_id} className="px-6 py-3 text-sm">
                                  <div className="flex items-start justify-between gap-3">
                                    <span className="flex items-start gap-2 font-medium leading-relaxed">
                                      <Check size={14} className="text-success shrink-0 mt-0.5" />
                                      {f.description}
                                    </span>
                                    <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide shrink-0">
                                      {REQUIREMENT_CATEGORY_LABELS[requirementCategory(f.requirement_type)]}
                                    </span>
                                  </div>
                                </li>
                              ))}
